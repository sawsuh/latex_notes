...
\begin{thm}
  ...
  Then, we can write:
  \begin{displaymath}
    \Delta u = \pdern ur2 + \pdern u\theta2 \frac{1}{r^2} + \pder ur\frac{1}{r}
  \end{displaymath}
\end{thm}
\begin{proof}[Proof:]
  First note:
...
\end{thm}
\begin{proof}
  First let us show that without loss of generality we can assume $\theta_0=0$.
  ...
  if the coefficients of $f$ are all 0 (which is an assumption of the theorem).
...
\begin{definition}[Good kernel]\label{def:goodkernel}
  ...
  \begin{enumerate}
    \item For all $n\in\NN$ \[ \frac1{2\pi}\int_{-\pi}^{\pi} K_n
      \left( t \right) \mathrm dt = 1 \]
    \item There exists $M>0$ such that for all $n\in\NN$ \[
...
\begin{lem}\label{lem:fejer_kernel_identity}
  \[
...
\begin{proof}[Proof of \cref{thm:fejer_good_kernels}:]
  ...
  \begin{align*}
    ...
    \shortintertext{by \cref{lem:fejer_kernel_identity}:}
